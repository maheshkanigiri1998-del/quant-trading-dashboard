...
export default function LiveTicker() {
  ...
  // FIX: Liquidation check moved into useEffect, NOT render body
  ...
  const fetchAISentiment = async () => {
    ...
    try {
      const response = await fetch('http://192.168.29.181:8000/api/sentiment');
    ...
    <div style={{ padding: '20px', fontFamily: 'monospace', border: isLiquidated ? '2px solid #ff4444' : '1px solid #333', maxWidth: '400px', backgroundColor: '#1e1e1e', color: '#fff', borderRadius: '8px' }}>
      ...
      <div style={{ padding: '15px', backgroundColor: '#2a2a2a', borderRadius: '6px', marginBottom: '15px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px', fontSize: '14px' }}>
          ...
          <span style={{ color: pnl >= 0 ? '#00ff00' : '#ff4444', fontWeight: 'bold' }}>${pnl.toFixed(2)}</span>
        </div>
      </div>

      <div style={{ padding: '15px', backgroundColor: '#1a237e', borderRadius: '6px', border: '1px solid #3f51b5' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
          <span style={{ color: '#8c9eff', fontWeight: 'bold', fontSize: '12px' }}>LLAMA 3 AI</span>
          <button onClick={fetchAISentiment} disabled={isThinking} style={{ padding: '5px 10px', backgroundColor: '#3f51b5', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>ANALYZE</button>
        </div>
        <div style={{ fontSize: '18px', fontWeight: 'bold', color: sentiment === 'BULLISH' ? '#00ff00' : '#ff4444' }}>{sentiment}</div>
      </div>
    </div>
  );
}